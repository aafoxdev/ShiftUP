...
if (!empty($_POST)) {
  // ログインの処理
  if (!empty($_POST['employeenumber']) && !empty($_POST['password'])) {
    $login = $db->prepare('SELECT * FROM members WHERE employeenumber=? AND password=?');
    $login->execute(
      array(
        $_POST['employeenumber'],
        sha1($_POST['password']) // 注意: セキュリティの観点から、より安全なパスワードハッシュ関数の使用を推奨します
      )
    );
    $member = $login->fetch();
            ...
            <dl class="form-area">
              ...
              <dd>
                <input type="text" class="input-text" name="employeenumber" size="35" maxlength="255"
                  value="<?php echo isset($_POST['employeenumber']) ? htmlspecialchars($_POST['employeenumber'], ENT_QUOTES) : ''; ?>" />
              ...
              <dd>
                <input type="password" class="input-text" name="password" size="35" maxlength="255"
                  value="<?php echo htmlspecialchars(isset($_POST['employeenumber']) ? $_POST['employeenumber'] : '', ENT_QUOTES); ?>"" />
             </dd>
                    </dl>
            <p class=" confirm-text">
                <input id="save" type="checkbox" name="save" value="on"><label for="save">次回からは自動的にログインする</label>
                </p>
                <p class="confirm-text"><a href="./admin-login.php">
                    <font color="blue">管理者はこちらへ</font>
                  </a></p>
